...
## 規模（見込み。根拠 = 2026-09-24 実測 `lib/api.ts` 554 行・58 メソッド）
- production ≈ 700 行: 10 ファイルへの移動 ≈ 554（移動であり diff は「削除＋追加」で 2 倍に見えるが内容は同一）、各ファイルの雛形（import・型）≈ 50、`lib/api.ts` の合成点 ≈ 80、`legacyRequest.ts`（TP1 を移す場合）≈ 40。
- test 0 行（1 行も変更しない。完了条件）。
- 合計 ≈ 700 行（実質の新規記述 ≈ 170 行）。
...
- baseline green: `npm test` / `npm run lint` / `npm run typecheck` / `npm run typecheck:ts7` / `npm run build`。着手前に次を記録する（2026-09-23 実測値は参考。着手時に数え直す）:
  ...
  ```
- Selection Gate 依存なし。
- 棄却済み案（再提案しない）: `createApiClient` factory の階層化（Spec §5 RO1）、context 別 6 分割をこの slice で行うこと（Spec §5 rejected_overdesign。W-S4d1）、`lib/api.ts` の削除（呼出側 21 ファイルの import 変更は W-S4d1 の page 側移行と同じ作業になり W-S1「呼出箇所は変更しない」に反する。削除は W-S4d3）。
...
- 既存テスト `tests/lib/api.{test,auth.test,invites.test,passkey.test,sessions.test,starred.test}.ts` 6 ファイルと `vi.mock('@/lib/api')` を使う 25 ファイルが **1 行も変更せず** green（本 slice 完了時点の条件。後続 W-S4d2a / W-S4d2b が 25 ファイルを、W-S4d3 が 6 ファイルを更新する（許可））。
