...
| wave 1 | [W-S1-gateway.md](W-S1-gateway.md) | `ApiGateway`（`Result` / `ApiFailure`・deadline）と最小注入点（再生系 4 箇所） | S0 | CI-T12 / T13 | 旧 `ApiError` 互換 adapter（TP1）を暫定保持（削除は W-S4d3） |
| 2 | [W-S1b-api-split.md](W-S1b-api-split.md) | `lib/api.ts` を backend リソース単位の 10 ファイルへ分割（横断関数 0 件・呼出側不変） | W-S1 | —（特性テストのみ） | 挙動不変（型・関数名不変） |
| 2' | [W-S2a-playback-domain.md](W-S2a-playback-domain.md) | `lib/playback/*` ドメイン層の新設 その 1（port・Session・Queue gate・source・ResumeRule・platform adapter）。既存コードから呼ばない | W-S1（W-S1b とは対象ファイルが重ならず並行可） | RS-01〜RS-07（純関数・表駆動）。Q-01〜Q-32 を新モジュールでも。CI-T1〜T4・T9 | ① 新規コードのみ |
| 2'' | [W-S2a2-playback-coordination.md](W-S2a2-playback-coordination.md) | その 2（OfflineLibrary・Coordinator・PositionReporter）。2026-09-24 に規模（≈ 1,900 行）で W-S2a から分けた | W-S2a（W-S1b と並行可） | CI-T5〜T8・T10・T11（PS-01〜PS-08） | ① 新規コードのみ |
| 3 | [W-S2b-playback-entry.md](W-S2b-playback-entry.md) | `PlaybackProvider` へ入口を差し替え。§2・Q-* は挙動不変。変わるのは列挙した行のみ | W-S2a ＋ W-S2a2 ＋ W-S1b ＋ W-0 | PS-01〜PS-06・PS-08（port double で全行）。§6.4 送信条件（PS-05 / 05b）。SG-X1 / X2 / X4。T-T18（TP2） | ② 特性テスト 13＋e2e 3（不変）＋準拠テスト（変更行） |
...
依存の直列: **W-S1 → {W-S1b ∥ (W-S2a → W-S2a2)} → W-S2b → W-S2c → {W-S3 ∥ W-S4a ∥ (W-S4b・W-S4c・W-S5 は順序不定の直列)} → W-S4d2a → W-S4d2b → W-S4d1 → W-S4d3**。W-0 は独立（W-S2b の前に merge）。W-S5 はさらに B-S5 の後（B-S5 は W-0 の後）。W-S4d2a は W-S4a の後なら W-S4b / W-S4c / W-S5 を待たずに投入できる。
...
| 1' | W-S2a2 | W-S2a の PR ＋ 親ポインタ | W-S1b |
| 2 | W-S2b | W-S1b・W-S2a・W-S2a2 の PR ＋ 親ポインタ、かつ W-0（wave 1）の PR ＋ 親ポインタ | backend B-S4 / B-S5（対象 module が違う） |
...
| 4 | W-S5 | W-S2c の PR ＋ 親ポインタ、かつ backend B-S5 の PR ＋ 親ポインタ | W-S3 / W-S4a / W-S4d2a。**W-S4b・W-S4c・W-S4d2b・W-S4d1・W-S4d3 とは直列**（認証 Provider・`settings/page.tsx`・`PlaybackProvider.tsx`・`AuthProvider.*.test.tsx` が重なる） |
