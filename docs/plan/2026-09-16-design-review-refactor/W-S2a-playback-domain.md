...
## 前提・着手条件
- 依存 slice: **W-S1**（`lib/api/gateway.ts` の `Result` / `ApiFailure` 型。Coordinator が注入で受けるのは gateway が返す `Result` 関数で、`createApiClient()` の throw 版ではない）の web PR が main に merge 済み、**かつ親リポ `news-listen` の submodule ポインタが進んでいる**（親で `git submodule status` の `web` 行に `+` が無い）こと。W-S1b（`lib/api.ts` の分割）とは対象ファイルが重ならないため、W-S1b の完了を待たずに並行投入できる（親 plan は直列表記だが、本 slice が `lib/api/*` を読むのは `gateway.ts` の型だけ。README「投入順」）。並行した場合、後から merge する側は rebase のみで conflict は生じない（新規ファイルのみ）。
