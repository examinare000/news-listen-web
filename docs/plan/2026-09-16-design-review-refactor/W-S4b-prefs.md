...
- W-S5 との順序は不定。W-S5 が先に merge 済みなら `lib/account/subjectCleanup.ts` の主体依存 3 key 定数を registry の宣言（下の対象 1）へ置き換え、定数を削除する。W-S5 が後なら registry が宣言だけ持ち、W-S5 がそれを読む（W-S5 order の表）。
...
- **`localStorage` の直接参照 0 件**（量化する集合 = `app/` `components/` `hooks/` `contexts/` `lib/` の `.ts` / `.tsx`。除外は次の 3 種だけ: `lib/platform/keyValueStore.ts`（adapter）、`app/layout.tsx:46` の inline theme script（TP3）、コメント行（2026-09-23 実測: `app/(app)/settings/page.tsx:38`）。`hooks/useLocalStorage.ts` は registry へ吸収するか adapter 経由に書き換え、`grep -rn "localStorage" app components hooks contexts lib` の残る出現が上の除外だけであることを PR 説明に列挙）。
...
- `Episode` の UI 展開・`error_message` 写像（W-S4a）、`PasswordPolicy`・`AuthSession`（W-S4c）、`lib/api` の context 別取り込み・page 側注入点の移行・TP1 削除（W-S4d）は行わない。`createApiClient()` の呼出箇所を変えない。
