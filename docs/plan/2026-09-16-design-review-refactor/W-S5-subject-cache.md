...
## 着手前に決める項目（`agent-rules/96`。1 件。決まるまで投入しない）
| # | 論点 | 選択肢 | 代償 | 決めないと止まる工程 |
|---|---|---|---|---|
| U-W5-1 | 起動時の認証解決が未確定（通信断・5xx・decode 失敗）に終わった後、利用者が login / register / passkey でログインに成功したとき、それを「主体の確定」に含めて回収 `reclaimAudioCaches(user_id)` を 1 回走らせるか | (A) 含める: 共有仕様 §6.3 性質 2「`authenticated` / 未認証に確定した時点で同じ回収を 1 回走らせる」の文言どおり。login 成功は未確定 → `authenticated` の確定である / (B) 含めない: 回収は起動時の `getMe` 解決（成功 / 401）だけで、未確定のまま login した場合は次回起動まで他主体の `audio-v1-*` が残る | (A) は login 経路に回収の呼出が 1 つ増える（冪等・await しない）。(B) は共有端末で「前主体の音声が次回起動まで残る」窓が生じる（主体別なので次の主体には見えないが、性質 2「離脱時に取りこぼした資産はここで確実に消える」が起動まで遅れる） | 対象 4 の発火条件・`tests/contexts/AuthContext*.test.tsx` の pin |

推奨は (A)（性質 2 の文言と SG-B3「主体確定時に 1 回」の自然な読み。router → user）。決定後、対象 4 の「確定」の定義に反映する。
...
- SL-01・SL-02・SL-04・SL-06・SL-07 が行 ID 付きで green（SL-01 / SL-07 は音声キャッシュ・端末設定・再生停止の全事後条件）。回収が主体確定ごとに 1 回だけ走り、通信断では走らないことをテストで pin。SL-06: 離脱後に完了した書き込みが A の Cache に入り、次回起動の回収後に `audio-v1-A` が存在しない。SL-07: B の確立が A の後始末を待たず、`audio-v1-B` は触られない。**遷移④で B の確立後に A のエピソードの `updatePosition` / `markCompleted` が gateway double に 1 回も来ない**（交差の pin。対象 4b）。
