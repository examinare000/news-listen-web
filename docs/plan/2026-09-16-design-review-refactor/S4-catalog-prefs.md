## web リファクタ S4: Catalog / Preferences / Account の残作業（学習機能サイクルで着手）
...
着手順 5（S2 に依存。学習機能サイクルで着手。backend S0b / S0c の merge が別途必要）。

## 前提・着手条件
- 依存 slice: S2（`Episode` 判別共用体・`nowPlaying()` が存在すること）が main に merge 済みであること。
- **着手条件**: backend の S0b / S0c（`PasswordPolicy` 12〜20 文字の ADR-101、`error_message` 識別子 4 値の ADR-102）が main に merge 済みであること。この着手条件が満たされるまで本 slice に着手しない。
...
- backend のパスワード検証値・`error_message` 4 値との整合を S4 着手時に確認する（Spec §3.3 OB-A1・§8 unknowns）。
- 棄却済み案（再提案しない、Spec §5 rejected_overdesign）: `lib/api` の context 別 6 分割を S1 で行うこと（本 slice で行う）。
...
6. **`lib/api` の context 別分割**: S1 では見送った分割（Playback / Catalog / Account / Preferences / Admin 等）をこの slice で行う。page 側 15 ファイル・37 箇所（`docs/research-reports/2026-09-16-code-design-review/verification-run.md` erratum の内訳）を新しい context 別 gateway 呼出へ移行する。TP1（`ApiError` 互換 adapter）の削除条件（`app/`・`components/`・`hooks/` が `ApiError` を import しなくなった時）をこの slice で満たし、TP1 を削除する。
